...
export class FloorComponent implements OnInit {
  ...
  columns = [
    ...
    downloadCSVfile(): void {
      ...
      this.downloadCSV(csvContent, 'Floor_log.csv');
  ...
  RemovedFloorList(){
    this.router.navigate(['/supervisor/removed-floor']);
    }
    ...
    getAllData() {
      this.service.getFloor().subscribe((res) => {
        ...
        // Sorting the data by updatedAt timestamp in descending order
        this.readData.sort((a: any, b: any) => {
          return new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime();
        });
    
        // Filter only enabled floors (disabled === false)
        this.enabledFloors = this.readData.filter((floor: any) => floor.disabled === false);
    
        this.totalLength = this.enabledFloors.length; // Update total length for pagination
    
        console.log("Enabled Floors:", this.enabledFloors); // Debugging output
      });
    }
...
OnEdit(us:any,form:any){
 ...
 FloorRemove(id:any){
  console.log(this.getparamid);
  const result = window.confirm('Are you sure you want to remove this Floor?');
  if (result) {
   this.loading = true;
    console.log('confirmed');
   this.service.removeFloor(
     {id:id } 
     ).subscribe((res)=>{
