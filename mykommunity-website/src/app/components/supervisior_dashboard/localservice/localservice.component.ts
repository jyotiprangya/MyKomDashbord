...
private mapColumnToKey(columnName: string): string {
  ...
  RemovedProviderList(){
    this.router.navigate(['/supervisor/removed-dailyhelp-provider']);
    }
  ...
  OnEdit(us:any,form:any){
   ...
   ServiceRemove(id:any){
    console.log(this.getparamid);
    const result = window.confirm('Are you sure you want to remove this Service Provider?');
    if (result) {
     this.loading = true;
      console.log('confirmed');
     this.service.removeServiceProvider(
       {id:id } 
       ).subscribe((res)=>{
