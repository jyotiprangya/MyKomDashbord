...
columns = [
  ...
  RemovedDailyhelpCategoryList(){
    this.router.navigate(['/supervisor/removed-dailyhelp-category']);
    }
...
this.thumbnail = this.sanitizer.bypassSecurityTrustUrl(this.myData);
  ...
  OnEdit(us:any,form:any){
   ...
   CategoryUpdate(){
    ...
    //  console.log(this.form.value,'updated');
     const result = window.confirm('Are you sure you want to change this Emergency Category Details?');
    if (result) {
      this.loading = true;
      console.log('confirmed');
     {
 ...
 ServiceRemove(id:any){
  console.log(this.getparamid);
  const result = window.confirm('Are you sure you want to remove this Dailyhelp Category?');
  if (result) {
   this.loading = true;
    console.log('confirmed');
   this.service.removeServiceCategory(
     {id:id } 
     ).subscribe((res)=>{
