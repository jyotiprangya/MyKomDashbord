...
RemovedBlockList(){
  this.router.navigate(['/supervisor/removed-block']);
  }
  ...
  getAllData() {
    this.service.getBlock().subscribe((res) => {
      ...
      this.enabledBlocks = this.readData.filter((block: any) => block.disabled === false);
  
      this.totalLength = this.enabledBlocks.length; // Update total length for pagination
  
      console.log("Enabled Blocks:", this.enabledBlocks); // Debugging output
    });
  }
...
OnEdit(us:any,form:any){
 ...
 BlockRemove(id:any){
  console.log(this.getparamid);
  const result = window.confirm('Are you sure you want to remove this Block?');
  if (result) {
   this.loading = true;
    console.log('confirmed');
   this.service.removeBlock(
     {id:id } 
     ).subscribe((res)=>{
...
UpdateButton(us:any){
  ...
  const result = window.confirm('Are you sure you want to delete this Block?');
    if (result) {
      this.loading = true;
      console.log('confirmed');
      console.log(us.disabled);
