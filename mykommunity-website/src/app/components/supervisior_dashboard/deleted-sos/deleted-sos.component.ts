...
@Component({
  selector: 'app-deleted-sos',
  templateUrl: './deleted-sos.component.html',
  styleUrls: ['./deleted-sos.component.css']
})
export class DeletedSosComponent implements OnInit {
  ...
  getAllRemovedData(){
    this.service.getRemovedSos().subscribe((res)=>{
      console.log(res,"res==>");
      this.readData = res.data;
      this.totalLength = (res.data).length;
    });
 ...
 SosRestore(id:any){
  ...
  const result = window.confirm('Are you sure you want to restore this SOS?');
  if (result) {
   this.loading = true;
    console.log('confirmed');
   this.service.restoreSos(
     {id:id } 
     ).subscribe((res)=>{
