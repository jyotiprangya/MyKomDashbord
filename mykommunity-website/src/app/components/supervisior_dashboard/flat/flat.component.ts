...
export class FlatComponent implements OnInit {
  ...
  RemovedFlatList(){
    this.router.navigate(['/supervisor/removed-flat']);
    }
...
OnEdit(us:any,form:any){
 ...
 FlatRemove(id:any){
  console.log(this.getparamid);
  const result = window.confirm('Are you sure you want to remove this Flat?');
  if (result) { 
   this.loading = true;
    console.log('confirmed');
   this.service.disableFlat(
     {id:id } 
     ).subscribe((res)=>{
 ...
 UpdateButton(us:any){
  ...
  const result = window.confirm('Are you sure you want to delete this flat?');
   ...
   console.log(us.disabled);
    ...
    this.service.disableFlat({id:this.getparamid}).subscribe((res)=>{
