...
@Component({
  selector: 'app-deleted-dailyhelp-category',
  templateUrl: './deleted-dailyhelp-category.component.html',
  styleUrls: ['./deleted-dailyhelp-category.component.css']
})
export class DeletedDailyhelpCategoryComponent implements OnInit {
  ...
  constructor(private service:ApiService,private fb: UntypedFormBuilder, private modalService: NgbModal, private _location: Location, private af:AngularFireStorage,
    private firebaseUploadService: FirebaseUploadService,    private sanitizer: DomSanitizer,private dialog : MatDialog,
    ) { }
  ...
  ngOnInit(): void {
    ...
    this.getRemovedAllCategoryData();
...
columns = [
  ...
  { name: 'Delete', visible: true },
    ...
    downloadCSVfile(): void {
      ...
      this.downloadCSV(csvContent, 'dailyhelpcategory_log.csv');
  ...
  getRemovedAllCategoryData(){
    this.service.getRemovedLocalServiceCategory().subscribe((res)=>{
      console.log(res,"res==>");
      this.readData = res.data;
     // console.log(this.getimageUrl);
  ...
  saveDetails(form:any) {
    const result = window.confirm('Are you sure you want to add this Service category?');
    if (result) {
      this.loading = true;
  ...
  if(this.form.valid)
  {
    ...
    this.service.createServiceCategory({name:this.form.value.name,image:this.firebase,societyId:this.societyId
    }).subscribe((res)=>{
      console.log(res,'res==>');
      alert("Local Service Category added successfully...");
      this.form.reset();
      this.getAllCategoryData();
      this.onCancelClick();
      this.loading = false;
   ...
   CategoryUpdate(){
    ...
    if (result) {
      ...
      console.log('confirmed');
     {
       ...
       this.service.updateServiceCategory({localServiceId:this.getparamid,name:this.form.value.name,image:this.firebase} ).subscribe((res)=>{
 ...
 ServiceRestore(id:any){
  console.log(this.getparamid);
  const result = window.confirm('Are you sure you want to restore this Dailyhelp Category?');
  if (result) {
   this.loading = true;
    console.log('confirmed');
   this.service.restoreServiceCategory(
     {id:id } 
     ).subscribe((res)=>{
