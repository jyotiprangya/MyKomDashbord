...
export class SosComponent implements OnInit {
  ...
  columns = [
    ...
    { name: 'Delete', visible: true },
        ...
        downloadCSVfile(): void {
          ...
          this.downloadCSV(csvContent, 'sostype_log.csv');
  ...
  getAllData(){
    this.service.getSos().subscribe((res)=>{
      console.log(res,"res==>");
 ...
 reader.onload = (event:any)=>{
  ...
  task
    ...
    RemovedSosList(){
      this.router.navigate(['/supervisor/removed-sos']);
      }
    
   saveDetails(form:any) {
    const result = window.confirm('Are you sure you want to add this SOS type in your Society?');
    if (result) {
      this.loading = true;
      console.log('confirmed');
    
       this.service.createSos({name:this.form.value.name,image:this.firebase,societyId:this.societyId
       }).subscribe((res)=>{
         console.log(res,'res==>');
         console.log('done')
         this.loading = false;
      ...
      OnEdit(us:any,form:any){
       ...
       SosUpdate(){
        ...
        //  console.log(this.form.value,'updated');
         const result = window.confirm('Are you sure you want to change this SOS Details?');
        if (result) {
          this.loading = true;
          console.log('confirmed');
         {
           
           this.service.updateSos({sosCategoryId:this.getparamid,name:this.form.value.name,image:this.firebase} ).subscribe((res)=>{
             alert( 'Updated Successfully...');
             console.log({sosCategoryId:this.getparamid,name:this.form.value.name,image:this.firebase});
               this.getAllData();
               this.loading = false;
               this.onCancelClick();
 ...
 SosRemove(id:any){
  console.log(this.getparamid);
  const result = window.confirm('Are you sure you want to remove this SOS?');
  if (result) {
   this.loading = true;
    console.log('confirmed');
   this.service.removeSos(
     {id:id } 
     ).subscribe((res)=>{
