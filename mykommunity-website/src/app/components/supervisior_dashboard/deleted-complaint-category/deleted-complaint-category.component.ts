...
@Component({
  selector: 'app-deleted-complaint-category',
  templateUrl: './deleted-complaint-category.component.html',
  styleUrls: ['./deleted-complaint-category.component.css']
})
export class DeletedComplaintCategoryComponent implements OnInit {
  closeResult: any;
  ...
  ngOnInit(): void {
   ...
   this. getAllRemovedCategoryData()
  ...
  getAllCategoryData(){
   ...
   getAllRemovedCategoryData(){
      this.service.getRemovedComplaintCategory().subscribe((res)=>{
        console.log(res,"res==>");
        this.readData = res.data;
        this.readData.sort((a:any, b:any) => {
          return new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime();
 ...
 CategoryRestore(id:any){
  ...
  const result = window.confirm('Are you sure you want to restore this category?');
  if (result) {
   this.loading = true;
    console.log('confirmed');
   this.service.restoreComplaintCategory(
     {id:id} 
     ).subscribe((res)=>{
