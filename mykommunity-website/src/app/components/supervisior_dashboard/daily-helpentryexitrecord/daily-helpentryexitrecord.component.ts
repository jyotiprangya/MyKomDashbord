...
@Component({
  selector: 'app-daily-helpentryexitrecord',
  templateUrl: './daily-helpentryexitrecord.component.html',
  styleUrls: ['./daily-helpentryexitrecord.component.css']
})
export class DailyHelpentryexitrecordComponent implements OnInit {
  ...
  sortingData(column: string): void {
  ...
  downloadCSVfile(): void {
    ...
    this.readData.forEach((entry: { [x: string]: any; records: any[]; }) => {
        ...
        entry.records.forEach((record: { inTime: any; outTime: any; }) => {
            ...
            visibleColumns.forEach(column => {
                ...
                if (key === 'inTime') {
                    ...
                    exportEntry['Exit Time'] = record.outTime;  // Add outTime
                } else if (key) {
...
private mapColumnToKey(columnName: string): string {
  ...
  getAllData() {
    this.service.getLocalServiceProviderEntryExitRecord().subscribe((res) => {
      console.log(res, "res==>");
  ...
  // This method will help to get sorted dates from records
  getSortedDates(groupedRecords: { [date: string]: any[] }): string[] {
