...
@Component({
  selector: 'app-emergencycategory',
  templateUrl: './emergencycategory.component.html',
  styleUrls: ['./emergencycategory.component.css']
})
export class EmergencycategoryComponent implements OnInit {
  closeResult: any;
...
RemovedEmergencyCategoryList(){
  this.router.navigate(['/supervisor/remove-emergencyCategory']);
  }
...
OnEdit(us:any,form:any){
 ...
 CategoryRemove(id:any){
  console.log(this.getparamid);
  const result = window.confirm('Are you sure you want to remove this Emergency Category?');
  if (result) {
   this.loading = true;
    console.log('confirmed');
   this.service.removeEmergencyCategory(
     {id:id } 
     ).subscribe((res)=>{
