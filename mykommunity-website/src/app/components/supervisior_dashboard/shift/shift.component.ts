...
RemovedShiftList(){
  this.router.navigate(['/supervisor/removed-shift-setup']);
}

getAllData() {
  this.service.getSecurityShifts().subscribe((res) => {
    this.readData = res.data.filter((shift: any) => shift.disabled === false);
...
ShiftRemove(id: any) {
  ...
  const result = window.confirm('Are you sure you want to remove this shift?');
  if (result) {
    this.loading = true;
    ...
    this.service.inactiveShift(payload).subscribe(
      (res) => {
