...
export class GateComponent implements OnInit {
  ...
  getAllData() {
    this.service.getGate().subscribe((res) => {
      ...
      // Filter only enabled gates (disabled === false)
      this.readData = res.data.filter((gate: any) => gate.disabled === false);
  
      this.totalLength = this.readData.length; // Update total length for pagination
  
      console.log("Enabled Gates:", this.readData); // Debugging output
    });
  }
  ...
  columns = [
    ...
    RemovedGateList(){
      this.router.navigate(['/supervisor/removed-gate']);
      }
...
OnEdit(us:any,form:any){
 ...
 GateRemove(id:any){
  console.log(this.getparamid);
  const result = window.confirm('Are you sure you want to remove this Gate?');
  if (result) {
   this.loading = true;
    console.log('confirmed');
   this.service.removedGate(
     {id:id } 
     ).subscribe((res)=>{
