...
@Component({
  selector: 'app-deleted-gate',
  templateUrl: './deleted-gate.component.html',
  styleUrls: ['./deleted-gate.component.css']
})
export class DeletedGateComponent implements OnInit {
  ...
  constructor(private service:ApiService,private fb: UntypedFormBuilder,
     private modalService: NgbModal, private _location: Location,private dialog: MatDialog,private router:Router,) { }
  ...
  getAllData() {
    this.service.getGate().subscribe((res) => {
      ...
      // Filter only disabled gates (disabled === true)
      this.disabledGates = res.data.filter((gate: any) => gate.disabled === true);
  
      this.totalLength = this.disabledGates.length; // Update total length for pagination
  
      console.log("Disabled Gates:", this.disabledGates); // Debugging output
    });
  }
  ...
  columns = [
    ...
    { name: 'Edit', visible: true },
      ...
      downloadCSVfile(): void {
        ...
        this.downloadCSV(csvContent, 'Gate_log.csv');
...
saveDetails(form:any) {
  const result = window.confirm('Are you sure you want to add this Gate?');
  if (result) {
...
OnEdit(us:any,form:any){
 ...
 UpdateGate(){
   ...
   const result = window.confirm('Are you sure you want to change this Gate Details?');
    if (result) {
      this.loading = true;
      console.log('confirmed');
   {
     this.service.updateGate({name:this.form.value.name, gateId:this.getparamid }).subscribe((res)=>{
 ...
 GateRestore(id:any){
  console.log(this.getparamid);
  const result = window.confirm('Are you sure you want to restore this Gate?');
  if (result) {
   this.loading = true;
    console.log('confirmed');
   this.service.restoredGate(
     {id:id } 
     ).subscribe((res)=>{
